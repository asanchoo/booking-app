...
- явное согласие на обработку данных с фиксацией даты и версии документов;
...
Публичные реквизиты оператора задаются при сборке frontend: `VITE_LEGAL_OPERATOR_NAME`, `VITE_LEGAL_OPERATOR_ID`, `VITE_LEGAL_ADDRESS`, `VITE_LEGAL_CONTACT`. Они отображаются в политике конфиденциальности и пользовательском соглашении, поэтому перед коммерческим запуском должны содержать реальные данные бизнеса. Безопасный шаблон находится в [`client/.env.example`](client/.env.example).
...
Перед подключением нескольких независимых салонов понадобятся multi-tenancy, журнал действий, резервное копирование и мониторинг ошибок. Для работы с реальными клиентами также необходимо заполнить реквизиты оператора и проверить размещение базы данных на соответствие требованиям применимого законодательства.
