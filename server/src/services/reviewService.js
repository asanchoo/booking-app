...
function getOwnedAttendedBooking(bookingId, clientPhone) {
...
export function createMasterReview({ bookingId, clientPhone, rating, comment = '', source = 'website' }) {
  ...
  const booking = getOwnedAttendedBooking(bookingId, clientPhone);
  ...
  try {
    const result = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(booking.id, booking.barberId, normalizePhone(clientPhone), rating, normalizedComment, source);
    return { id: Number(result.lastInsertRowid), bookingId: booking.id, rating, comment: normalizedComment, source };
  } catch (error) {
    if (String(error?.message).includes('UNIQUE constraint failed: barber_reviews.booking_id')) {
...
export function updateTelegramReviewComment({ bookingId, clientPhone, comment }) {
  getOwnedAttendedBooking(bookingId, clientPhone);
  const normalizedComment = String(comment || '').trim().slice(0, 500);
  ...
  `).run(normalizedComment, bookingId, normalizePhone(clientPhone));
